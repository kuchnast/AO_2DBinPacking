...
class OpenedBin:
    ...
    ceiling_levels: List[List[int]] = field(
        default_factory=list)  # available levels on ceiling for bin list((width, height))


class LongerSideFirstAlgorithm(OnlineAlgorithm):
    ...
    @staticmethod
    def get_name():
        return "Longer Side First Algorithm"
    ...
    def _open_bin(self):
        self.opened_bins.append(
            OpenedBin(Bin2D(self.bin_width, self.bin_height), [[0, 0]], [[self.bin_width, self.bin_height]]))
    ...
    def _pack(self, package: Package2D) -> None:
        ...
        for ob in self.opened_bins:  # loop over all levels of all bins
            for level_idx in range(len(ob.levels)):
                ...
                if left_width is not None:
                    if left_width == 0:  # package left no free space on this level, so it is just the best choice
        ...
        if width_left_for_best_loc is not None:  # best package found with minimal loss
            self._pack_in_best_loc(best_loc, package)
            return

        # if no free space in other boxes, create a new one
        self._open_bin()
        ...
        if self._insert_if_fit(self.opened_bins[-1], Point2D(width, height), package):
            if height + package.height < self.opened_bins[-1].bin.height:
                # if a new level can be added, create a new one above
    ...
    def run(self) -> int:
        # Retrieve all packages from the data generator
        packages = []
        ...
        # Sort the packages in decreasing order of the long side
        sorted_packages = sorted(packages, key=lambda p: max(p.width, p.height), reverse=True)
